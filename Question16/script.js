document.addEventListener("DOMContentLoaded", function() {
    ...
    if (submitButton) {
        submitButton.addEventListener("click", function() {
			...
			if (selectedAnswer.value === "A") {
				feedback.textContent = "✔ Correct! Effectiveness ensures that each step of the algorithm is simple enough to be executed, guaranteeing that the algorithm can be carried out in practice.";
				feedback.className = "correct";
			} else {
				feedback.textContent = "✘ Incorrect. The correct answer is A: Effectiveness ensures that each step of the algorithm is simple enough to be executed, guaranteeing that the algorithm can be carried out in practice.";
				feedback.className = "wrong";
			}
        });
    } else {
    ...
    if (backButton) {
        backButton.addEventListener("click", function() {
            ...
            window.location.href = "../Question15/index.html";
        });
    } else {

    ...
    if (submitButton) {
        submitButton.addEventListener("click", function() {
			...
			if (selectedAnswer.value === "C") {
				feedback.textContent = "✔ Correct! A binary tree is a tree where each node has up to two children (left and right).";
				feedback.className = "correct";
			} else {
				feedback.textContent = "✘ Incorrect. The correct answer is C: A binary tree is a tree where each node has up to two children (left and right).";
				feedback.className = "wrong";
			}
        });
    } else {

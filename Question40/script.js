document.addEventListener("DOMContentLoaded", function() {
    ...
    if (submitButton) {
        submitButton.addEventListener("click", function() {
			...
			if (selectedAnswer.value === "A") {
				feedback.textContent = "✔ Correct! Appending an item to an array without resizing is a constant-time operation, O(1), because it involves adding the element to the next available index.";
				feedback.className = "correct";
			} else {
				feedback.textContent = "✘ Incorrect. The correct answer is A: Appending an item to an array without resizing is O(1) because it simply adds the element to the next available index.";
				feedback.className = "wrong";
			}
        });
    } else {
    ...
    if (nextButton) {
        nextButton.addEventListener("click", function() {
            ...
            window.location.href = "../Question41/index.html";
        });
    } else {
